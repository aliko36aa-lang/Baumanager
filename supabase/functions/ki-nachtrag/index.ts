...
Deno.serve(async (req) => {
  ...
  try {
    const { text, foto } = await req.json()
    ...
    // Build message content – optionally with image
    const content: unknown[] = []
    if (foto && typeof foto === 'string' && foto.startsWith('data:image')) {
      const [header, base64] = foto.split(',')
    ...
    content.push({ type: 'text', text: String(text).slice(0, 3000) })
    ...
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1500,
        system:
          'Du bist Kalkulator für Nachtragsarbeiten in einem deutschen Bauunternehmen. ' +
          'Der Nutzer beschreibt Mehraufwand oder Änderungen auf der Baustelle. ' +
          'Erstelle realistische Nachtrag-Positionen mit marktüblichen Netto-Preisen (Deutschland). ' +
